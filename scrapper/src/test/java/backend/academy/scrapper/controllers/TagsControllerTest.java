...
import backend.academy.scrapper.LinksAddRemoveTest;
...
class TagsControllerTest {
    ...
    void deactivate_userNotFound_shouldThrow() {
        assertThrows(NotFoundException.class,
            () -> tagsController.deactivate(new TagsRequest(777, "tag")));
    }

    @Test
    void deactivate_tagInactive_shouldStayInactive() {
        userRepository.save(new User(2));
        assertTrue(tagsController.deactivate(new TagsRequest(2, "tag")).getStatusCode().is2xxSuccessful());
    ...
    void deactivate_tagActive_shouldDeactivate() {
        ...
        assertTrue(tagsController.deactivate(new TagsRequest(3, "tag")).getStatusCode().is2xxSuccessful());
    ...
    void reactivate_tagInactive_shouldReactivate() {
        userRepository.save(new User(2));
        assertTrue(tagsController.reactivate(new TagsRequest(2, "tag")).getStatusCode().is2xxSuccessful());
    ...
    void reactivate_tagActive_shouldStayActive() {
        userRepository.save(new User(3, List.of(), List.of("tag")));
        assertTrue(tagsController.reactivate(new TagsRequest(3, "tag")).getStatusCode().is2xxSuccessful());
