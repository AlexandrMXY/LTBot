...
public class LinksAddRemoveTest {
    ...
    public void addLinks_addGithubLink_success() {
        ...
        TrackedLink link = u.links().stream()
            .filter(l -> l.serviceId().equals("aaa/bbb") && l.monitoringService().equals("githubMonitor")).findAny().orElseThrow();
    ...
    public void addLinks_addStackoverflowLink_success() {
        ...
        TrackedLink link = u.links().stream().filter(l ->
            l.serviceId().equals("14938748") && l.monitoringService().equals("stackoverflowMonitor")).findAny().orElseThrow();
    ...
    public void addLink_duplicatePassed_alreadyExistsException() {
        linksController.addLinks(0,
            new AddLinkRequest("https://github.com/qwerty/qwerty", List.of(), List.of()));
        assertThrows(
