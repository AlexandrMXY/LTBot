...
public class StackoverflowMonitor implements LinkMonitor {
    ...
    public UpdateResult checkForUpdates() {
        ...
        List<TrackedLink> links = linkRepository.findAllByMonitoringService(MONITOR_NAME);
        ...
        var updates = stackoverflowService.getUpdates(
            links.stream().map(link -> Long.parseLong(link.serviceId())).toList(),
            monitorData
                .orElseThrow(() -> new IllegalStateException("Stackoverflow monitor data not found"))
                .lastUpdate());

        log.info("Found updates for {}", updates);

        List<TrackedLink> updatedLinks = linkRepository.findAllByMonitoringServiceAndServiceIdIn(
            MONITOR_NAME,
            updates.stream().map(update -> String.valueOf(update.questionId())).toList());
        ...
        updatedLinks.forEach((upd) -> {
            res.addUpdate(new UpdateResult.UpdateDetails(
                upd.user().id(),
                upd.url(),
                "Updated"
            ));
        ...
        monitoringServiceDataRepository.save(
            monitorData.orElseThrow().lastUpdate(System.currentTimeMillis() / 1000L));
        return res;
    }
}
