...
class AsyncScrapperServiceTest {
    ...
    public void resetResilience4j() {
        circuitBreakerRegistry
                .circuitBreaker(AsyncScrapperService.RESILIENCE4J_INSTANCE_NAME)
                .reset();
    }
    ...
    public void initWiremock() {
        var portMatcher =
                Pattern.compile("(?:https?://)?localhost:(?<port>\\d{1,6})").matcher(config.scrapperUrl());
    ...
    public Stream<Arguments> methods() {
        return Stream.of(
                Arguments.of(new MethodDescription(
                        RequestMethod.POST, "/tags/deactivate", () -> service.deactivateTag(1L, ""))),
                Arguments.of(new MethodDescription(
                        RequestMethod.POST, "/tags/reactivate", () -> service.reactivateTag(1L, ""))),
                Arguments.of(new MethodDescription(RequestMethod.GET, "/tags/1", () -> service.getTagsList(1L))),
                Arguments.of(new MethodDescription(
                        RequestMethod.POST,
                        "/links",
                        () -> service.trackRequest(1L, new AddLinkRequest("", List.of(), List.of())))),
                Arguments.of(new MethodDescription(
                        RequestMethod.DELETE, "/links", () -> service.removeLink(1L, new RemoveLinkRequest("")))),
                Arguments.of(new MethodDescription(RequestMethod.POST, "/tg-chat/1", () -> service.registerChat(1L))),
                Arguments.of(new MethodDescription(RequestMethod.GET, "/links", () -> service.getTrackedLinks(1L))),
                Arguments.of(new MethodDescription(
                        RequestMethod.POST, "/links/tags", () -> service.addTagToLink(1L, "", ""))),
                Arguments.of(new MethodDescription(
                        RequestMethod.DELETE, "/links/tags", () -> service.removeTagFromLink(1L, "", ""))),
                Arguments.of(new MethodDescription(
                        RequestMethod.GET, "/tags/linksWithTag", () -> service.getLinksWithTag(1L, ""))),
                Arguments.of(new MethodDescription(
                        RequestMethod.POST,
                        "/notifications/1/policy",
                        () -> service.setNotificationPolicy(
                                1L, new NotificationPolicy(NotificationPolicy.INSTANT, 100)))),
                Arguments.of(new MethodDescription(
                        RequestMethod.GET, "/notifications/1/policy", () -> service.getNotificationPolicy(1L))));
    }
    ...
    public void request_on5xxErrorResponse_retry(MethodDescription description) {
        ...
        var response = description.requestCaller().get();
        response.doOnError((t) -> log.atWarn().setCause(t).log())
                .onErrorComplete()
                .block();
    ...
    public void request_waitLimitReached_error(MethodDescription description) {
        ...
        assertThatThrownBy(() -> description.requestCaller().get().block()).hasCauseInstanceOf(TimeoutException.class);
    }
    ...
    public record MethodDescription(
        ...
        MethodDescription(RequestMethod method, String urlRegex, Supplier<Mono<?>> caller) {
            this(
                    request(String.valueOf(method), urlMatching(urlRegex)),
