...
public class StartCommand extends AbstractSimpleRequestCommand {
    public StartCommand(TelegramService telegramService, AsyncScrapperService scrapperService, SessionStateManager sessionStateManager) {
        super((message) -> {
            if (!"/start".equals(message.message().trim())) {
                telegramService.sendMessage(message.chatId(), "Invalid command");
                return false;
            }
            scrapperService.registerChat(message.chatId()).subscribe(
                success -> sessionStateManager.onUpdate(message.chatId(), new SuccessResponseEvent("Welcome")),
