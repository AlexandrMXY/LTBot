...
public class UntrackCommand extends AbstractSimpleRequestCommand {
    public UntrackCommand(TelegramService telegramService, AsyncScrapperService scrapperService, SessionStateManager sessionStateManager) {
        super((message) -> {
            ...
            String url = msg[1];
            scrapperService.removeLink(message.chatId(), new RemoveLinkRequest(url))
                .subscribe(
                    (response -> sessionStateManager.onUpdate(message.chatId(), new LinkResponseEvent(response))),
