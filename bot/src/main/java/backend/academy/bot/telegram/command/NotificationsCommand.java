...
public class NotificationsCommand extends AbstractSimpleRequestCommand {
                ...
                (message) -> {
                    ...
                    if (args.length == 2) {
                        if (args[1].equals("current")) {
                            scrapperService
                                    .getNotificationPolicy(message.chatId())
                                    .subscribe(
                                            notificationPolicy -> sessionStateManager.onUpdate(
                                                    message.chatId(),
                                                    new SuccessResponseEvent(
                                                            getNotificationPolicyMessage(notificationPolicy))),
                                            t -> sessionStateManager.onUpdate(
                                                    message.chatId(), new ErrorResponseEvent(t)));
                            return true;
                        } else if (args[1].equals("instant")) {
                            scrapperService
                                    .setNotificationPolicy(
                                            message.chatId(), new NotificationPolicy(NotificationPolicy.INSTANT, -1))
                                    .subscribe(
                                            success -> sessionStateManager.onUpdate(
                                                    message.chatId(), new SuccessResponseEvent("Success")),
                                            t -> sessionStateManager.onUpdate(
                                                    message.chatId(), new ErrorResponseEvent(t)));
                            return true;
                        }
                    } else if (args.length == 3 && args[1].equals("delayed")) {
                        ...
                        scrapperService
                                .setNotificationPolicy(
                                        message.chatId(), new NotificationPolicy(NotificationPolicy.DELAYED, time))
                                .subscribe(
                                        success -> sessionStateManager.onUpdate(
                                                message.chatId(), new SuccessResponseEvent("Success")),
    ...
    private static String getNotificationPolicyMessage(NotificationPolicy policy) {
        if ("INSTANT".equals(policy.strategy())) return "Notifications are sent immediately";
